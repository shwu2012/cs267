...
public class DataPrep {
	...
	private static void RawMSLRToCSV(String inputFilePath, String outputFilePath)
		...
		while (line != null) {
			...
			outputLine += split[0]; // rating as the first column
	...
	private static void csvRelevanceStats(String inputFilePath)
		...
		for (int rating = 0; rating < 5; rating++) {
			...
			for (int query = 0; query < numQueries; query++) {
				...
				if (ratingCount[query][rating] > max)
					max = ratingCount[query][rating];
				if (ratingCount[query][rating] < min)
					min = ratingCount[query][rating];
			}
			System.out
					.println("Rating " + rating + ", min: " + min + ", max: "
							+ max + ", total: " + total + " avg: " + total
							/ numQueries);
	 ...
	 * 
	 */
	private static void sampleMSLRcsvForPairwise(int n, String inputFilePath,
	...
	private static void generatePairwiseOutput(String inputFilePath,
		...
		System.out
				.println("Processing Pairwise Output, this can take some time, as message "
