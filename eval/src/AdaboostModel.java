...
public class AdaboostModel implements MSLRbinaryModel{
  ...
  private final static String MODEL_FILE = "model.txt";
  private final ArrayList<ArrayList<WeakClassifier>> model = new ArrayList<ArrayList<WeakClassifier>>(10);
  ...
  public int predict(String difference) {
    ArrayList<Double> featureValues = getFeatureValues(difference);
    int numRows = model.size();
    int numCols = model.get(0).size();
    ArrayList<Integer> groupVotes = new ArrayList<Integer>(numCols);
    ArrayList<Double> groupWeights = new ArrayList<Double>(numCols);
    for(int col = 0; col < numCols; col++){
      ...
      groupWeights.add(weight / numRows);
    }
  ...
  public ArrayList<Double> getFeatureValues(String difference){
    ArrayList<Double> featureValues = new ArrayList<Double>(136);
    String[] s = difference.split(",");
    for (int i = 0; i < 136; i++){
      featureValues.add(Double.parseDouble(s[i+1]));
    }
